...
class GetMerchantClass(unittest.TestCase):
    ...
    def test_getmerchant_2(self):
        '''登录后，检查返回merchant_id 不为空'''
        for i in self.testcase_index:
            if self.scenario[i] =='检查返回merchant_id不为空':
                self.print_start_info(i, self.headers)
                api = testApi(self.method[i], self.url[i], self.params[i], self.headers)
                api_json = api.getJson()
                self.assertIsNotNone(api_json['MerchantId'])
                self.print_end_info(i)

    def test_getmerchant_3(self):
        '''检查merchant的状态是Published'''
        for i in self.testcase_index:
            if self.scenario[i] =='检查merchant的状态是Published':
                self.print_start_info(i, self.headers)
                api = testApi(self.method[i], self.url[i], self.params[i], self.headers)
                # api_return = api.testingApi()
                MerchantStatus = api.getJson()['MerchantStatus']
                self.assertEqual(MerchantStatus,self.expect[i])
                print(self.expect[i])
                self.print_end_info(i)
